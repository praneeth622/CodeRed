  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-purple-100 to-indigo-100">
      ...
      >
        ...
        <div className="absolute inset-0 flex flex-col items-center justify-center text-center px-4">
          ...
          <motion.button
            ...
            className="px-6 py-3 bg-white text-indigo-600 rounded-lg hover:bg-indigo-100 transition-colors duration-300 text-lg font-semibold"
      ...
      >
        ...
        >
          {[
            { icon: <Music className="w-6 h-6" />, text: "Live performances by top artists" },
            { icon: <Star className="w-6 h-6" />, text: "Talent showcases and competitions" },
            { icon: <Users className="w-6 h-6" />, text: "Interactive workshops and seminars" },
            { icon: <MapPin className="w-6 h-6" />, text: "Food courts with diverse cuisines" }
          ].map((item, index) => (
            <motion.li
              key={index}
        ...
        >
          {[
            { date: "March 15", event: "Opening Ceremony" },
            { date: "March 16-17", event: "Cultural Competitions" },
            { date: "March 18", event: "Celebrity Performance Night" },
            { date: "March 19", event: "Closing Ceremony & Awards" }
          ].map((item, index) => (
            <motion.div
              key={index}
